    ...
    methods: {
      ...
      addcommdevice (row) {
        ...
        if(row) {
          ...
          this.$http({
            url: this.$http.adornUrl(`/admin/tcommequip/info/${row.commId}`),
            method: 'get',
            data: this.$http.adornData()
          }).then(({data})=>{
            if(data && data.code === 0) {
              this.dialogTitle = data.tCommEquip.commId
              this.dataFormSet.setSitevalue = data.tCommEquip.stationId.toString()
              this.dataFormSet.channelName = data.tCommEquip.channelId.toString()
              this.dataFormSet.equipTypeId = data.tCommEquip.equipTypeId
              this.dataFormSet.commEquipName = data.tCommEquip.commEquipName
              this.dataFormSet.remark = data.tCommEquip.remark
              this.dataFormSet.commRelation = data.tCommEquip.commRelation.toString()
              this.dataFormSet.commAddress = data.tCommEquip.commAddress
              this.dataFormSet.localConnectUrl = data.tCommEquip.localConnectUrl
              this.dataFormSet.localPort = data.tCommEquip.localPort
              this.$http({
                url: this.$http.adornUrl('/admin/tstation/orgName'),
                method: 'post',
                data: this.$http.adornData({
                  orgId: data.tCommEquip.orgId
                })
              }).then(({data}) => {
                if (data && data.code === 0) {
          ...
          this.$nextTick(function () {
            ...
            // this.dataFormSet.commRelation = row.commRelation.toString()
