<template>
  <div id="tyxrule" class="tyxrule">
    <el-row class="tyxrule-item">
      <!-- 遥信配置表格 -->
      <el-col :xl="11" :lg="11" class="tyxrule-item">
        <el-form :inline="true" :model="dataForm" @keyup.enter.native="getDataList()" class="yx-form">
          <el-form-item>
            <el-select
        ...
        <el-table
          ...
          <el-table-column fixed="right" header-align="center" align="center" min-width="100" label="操作">
            <template slot-scope="scope">
              <el-button :plain="true" type="warning" size="mini" @click="addOrUpdateHandle(scope.row.yxId)">配置</el-button>
            </template>
          </el-table-column>
        </el-table>
      ...
      <!-- 转发遥信表格 -->
      <el-col :xl="13" :lg="13" class="tyxrule-item yxsortable">
...
<script>
import AddOrUpdate from './tyxrule-add-or-update'
...
export default {
  ...
  created () {
    this.yxtableHeight = window.innerHeight - 320    
  },
  activated() {   
  },
  mounted () {
    ...
    document.getElementById('tyxrule').style.height = (window.innerHeight - 170) + 'px'
    window.onresize = function () {
      if(document.getElementById('tyxrule')){
        document.getElementById('tyxrule').style.height = (window.innerHeight - 170) + 'px'
      }
    } 
  },
  methods: {
    ...
    getLists () {     
      if(this.dataForm.channelSelected !== '') {
        this.$http({
          url: this.$http.adornUrl('/base/tyxrule/saveAuto'),
          method: 'post',
    ...
    getDataList() {
      ...
      this.$http({
        url: this.$http.adornUrl('/base/tyxrule/list'),
        method: 'post',
        data: this.$http.adornParams({
    ...
    forwardHandle () {
      if(this.zfDataForm.channelSelected === '') {
        ...
        this.$confirm('确定进行转发操作?', '提示', {
          ...
          console.log(ids)
          this.$http({
            url: this.$http.adornUrl('/base/tzfyxrule/saveList'),
            method: 'post',
                ...
                onClose: () => {
                  ...
                  for(let i=0; i<this.dataList.length; i++) {
                    for(let j=0; j<this.dataListSelections.length; j++){
                      if(this.dataListSelections[j].yxAddress == this.dataList[i].yxAddress){
    ...
    zfGetDataList () {
      ...
      this.$http({
        url: this.$http.adornUrl('/base/tzfyxrule/list'),
        method: 'post',
        data: this.$http.adornParams({
    ...
    sortNumber (data) {
      ...
      if (nextOrPreviousId != null) {
        ...
        this.$http({
          url: this.$http.adornUrl('/base/tzfyxrule/insertionSort'),
          method: 'post',
    ...
    zfDeleteHandle (val) {
      ...
      this.$confirm(`确定进行[${id ? '删除' : '批量删除'}]操作?`, '提示', {
        ...
        this.$http({
          url: this.$http.adornUrl('/base/tzfyxrule/delete'),
          method: 'post',
