<template>
    <div class="non_urgent" id="non_urgent">
      <div class="inform">
      ...
      <div class="power-top">
        ...
        <el-input
          ...
          placeholder="100kw"
          ></el-input>
        <div class="but" @click="non_urgentExecute()">互动</div>
        <div class="but but-on" @click="non_urgentSimulation()">仿真</div>
        <div class="clear"></div> -->
      </div>
      <div class="power-echarts" id="power-echarts"></div>
      <div class="power-foot-echarts">
        <!-- <div class="power-foot-title">
          ...
          <div >
            <span v-for="item in checkLists" :key="item.id" class="sysname">{{item.name}}</span>
          </div>
          
        </div> -->
        <div class="foot-echarts" id="foot-echart"></div>
      </div>
      ...
      <el-dialog title="" :visible.sync="dialogTableVisible">
        <el-table :data="gridData" :header-cell-style="dailogGetRowClass" border stripe>
          ...
          <el-table-column property="actionVals" label="调控量" width="120" align="center"></el-table-column>
        ...
        <el-pagination  @size-change="handleSizeChange" @current-change="handleCurrentChange" :current-page="currentPage" background
          layout="prev, pager, next" :total="totals" :page-sizes="pagesize" :page-count="totalPage">
        </el-pagination>
      </el-dialog>
  ...
  <script>
    export default {
      ...
      mounted() {
        document.getElementById('non_urgent').style.height = (window.innerHeight - 170) + 'px'
        this.$http({
          url: this.$http.adornUrl('/device/stategrid/exigence/chart/hourload'),
          method: 'post',
        }).then(({
          data
        }) => {
          if (data.msg === "success") {
            let arr = [];
                    ...
                    arr.push("24:00")
            // this.energyXAxis = JSON.parse(data.chart.xAxis);
        ...
        //紧急需求响应子系统
        this.$http({
          url: this.$http.adornUrl('/device/stategrid/exigence/chart/system'),
          method: 'post',
        }).then(({
          data
        }) => {
          if (data.msg === "success") {
            ...
            // this.checkList= (data.systemIds ==false?[1,2]:data.systemIds)
            ...
            //   // this.checkLists[i].id=i+1
            // }
            
            this.initSubsystemChart(arr)
            // this.checkList[1]
          }
        })
      //   //多选按钮
      ...
      // })
        ...
        window.onresize = function () {
          if (document.getElementById('non_urgent')) {
            document.getElementById('non_urgent').style.height = (window.innerHeight - 170) + 'px'
            echarts.init(document.getElementById('power-echarts')).resize();
            echarts.init(document.getElementById('foot-echart')).resize();
          }
        }
      },
      destroyed() {
      ...
      methods: {
        ...
        initEnergyChart() {
          let _this = this
          var electricity = echarts.init(document.getElementById('power-echarts'));
          var option = {
            toolbox: {
              show: false
            },
          ...
          electricity.setOption(option);
              ...
              if ('WebSocket' in window) {
                ...
                _this.ws1 = new WebSocket(window.SITE_CONFIG.wsBaseUrl + token + "/exigence" + "/curve");
                ...
                _this.ws1.onmessage = function (evt) {
                  ...
                  for (let i = 0; i < _this.energySeries.length; i++) {
                    if (_this.energySeries[i].id == data.id) {
                      ...
                      // console.log(_this.energySeries[i].data)
                    }
                  }
                  electricity.setOption(option);
                };
                _this.ws1.onclose = function (evt) {
        ...
        initSubsystemChart(arr) {
          var electricity = echarts.init(document.getElementById('foot-echart'));
          let color= ['#ff110f', '#9b9eeb', '#c46edd']
          var option = {
            title: {
              text: ' '
            },
            ...
            xAxis: {
              boundaryGap: false,
              type: 'category',
              // data: this.subsystemXAxis,
              // data: (this.subsystemXAxis==null?arr:JSON.parse(this.subsystemXAxis)),
            ...
            series: [{
                // name: this.subsystemLegend[0],
                // data: this.subsystemSeries[0].data,
              ...
              //   name: (this.subsystemSeries==null?[{name:'VRV空调系统',data:[]}]:this.subsystemSeries)[1].name,
              // data: (this.subsystemSeries==null?[{name:'VRV空调系统',data:[]}]:this.subsystemSeries)[1].data,
              //   type: 'line',
              //   smooth: true,
              //   itemStyle: {
              //     normal: {
...
/*         // 点击表格删除按钮提示
  ...
  <style>
    ...
    .non_urgent .power-echarts {
      width: 95%;
      min-height: 50%;
      margin-left: 5%;
    }
  
    .non_urgent .power-foot-echarts {
      width: 40%;
      height: 45%;
      display: inline-block;
    ...
    .non_urgent .foot-echarts {
      width: 90%;
      height: 80%;
      margin-left: 10%;
    }
  
    .non_urgent .foot-table {
      width: 50%;
      height: 40%;
      display: inline-block;
      vertical-align: middle;
    ...
    .non_urgent .layer-page {
      text-align: center;
      margin: 1% 0 0 0;
    }
  
    .non_urgent .file-list {
      width: 5%;
      height: 8%;
      border: 1px solid #ccc;
      border-radius: 5px;
    ...
    .non_urgent .data-list {
      width: 100%;
    }
  
    .non_urgent .data-list div {
      float: left;
    ...
    .non_urgent .list-name {
      width: 25%;
      padding: 1% 1%;
    }
  
    .non_urgent .list-sdate {
      width: 15%;
      padding: 1% 1%;
    }
  
    .non_urgent .list-odate {
      width: 15%;
      padding: 1% 1%;
    }
  
    .non_urgent .list-size {
      width: 12%;
      padding: 1% 1%;
    }
  
    .non_urgent .list-revise {
      width: 11%;
      padding: 1% 1%;
    }
  
    .non_urgent .revise-bg {
      background: url(~@/assets/images/revise-bg.png) 50% 50% no-repeat;
      padding: 1.6% 1%;
      cursor: pointer;
    }
  
    .non_urgent.list-one div {
      background-color: #d8ebfa;
      color: #000;
    }
  
    .non_urgent .list-two div {
    ...
    .non_urgent .name-padding {
      padding: 0.9% 1%;
    }
  
    .non_urgent .layer-tips {
      border: 0;
      background-color: #fff;
      color: #000;
      text-align: center;
    ...
    .non_urgent .el-table td,
    .non_urgent .el-table th.is-leaf {
      border-color: #000
    }
  
    .non_urgent .el-table--border {
      border-color: #666666;
      border-radius: 10px
    }
  
    .non_urgent .el-table__body-wrapper {
      /*  overflow-y: scroll */
    }
    ...
    .non_urgent .el-dialog__body {
      background-color: #eeeeee;
      text-align: center
    }
  
    .non_urgent .el-dialog__body p {
      letter-spacing: 1px
    }
  
    .non_urgent .el-dialog__title {
      font-weight: 600;
      letter-spacing: 1px
    }
    ...
    .non_urgent .el-table__header {
      border: none;
    }
  
    .non_urgent .el-table--border::after,
    .non_urgent .el-table--group::after {
      top: 0;
      right: 0;
      width: 0px;
      height: 0%;
    ...
    .non_urgent .foot-table table thead tr th:nth-last-child(2),
    .non_urgent .foot-table table tbody tr td:nth-last-child(1) {
      border-right: none
    }
