<template>
  <div id="tytrule" class="tytrule">
    <el-row class="tytrule-item">
      <!-- 遥调配置表格 -->
      <el-col :xl="11" :lg="11" class="tytrule-item">
        <el-form :inline="true" :model="dataForm" @keyup.enter.native="getDataList()" class="yt-form">
          <el-form-item>
            <el-select
        ...
        <el-table
          ...
          <!-- <el-table-column fixed="right" header-align="center" align="center" width="150" label="操作">
            <template slot-scope="scope">
              <el-button :plain="true" type="warning" size="mini" @click="addOrUpdateHandle(scope.row.ytId)">配置</el-button>
            </template>
          </el-table-column> -->
        </el-table>
      ...
      <!-- 转发遥调表格 -->
      <el-col :xl="13" :lg="13" class="tytrule-item ytsortable">
...
<script>
import AddOrUpdate from './tytrule-add-or-update'
...
export default {
  ...
  created () {
    this.yttableHeight = window.innerHeight - 320
  },
  activated() {    
  },
  mounted () {
    ...
    document.getElementById('tytrule').style.height = (window.innerHeight - 170) + 'px'
    window.onresize = function () {
      if(document.getElementById('tytrule')){
        document.getElementById('tytrule').style.height = (window.innerHeight - 170) + 'px'
      }
    } 
  },
  methods: {
    ...
    // 遥调获取数据列表
    getDataList() {
      console.log(this.dataForm.channelSelected)
      this.dataListLoading = true
      this.$http({
        url: this.$http.adornUrl('/base/tytrule/list'),
        method: 'post',
        data: this.$http.adornParams({
        ...
        this.dataListSelections.map(item => {
          if(item.isExist === '0') {
            let obj = {
              ...
              "zfYtAddress": this.zfDataForm.channelSelected,
              "srcChannelId": item.channelId,
              "srcYtAddress": item.ytAddress
            }
            arr.push(obj)
            ids.push(item.ytId)
          }          
        })
        this.$confirm('确定进行转发操作?', '提示', {
          ...
          console.log(ids)
          this.$http({
            url: this.$http.adornUrl('/base/tzfytrule/saveList'),
            method: 'post',
                ...
                onClose: () => {
                  ...
                  for(let i=0; i<this.dataList.length; i++) {
                    for(let j=0; j<this.dataListSelections.length; j++){
                      if(this.dataListSelections[j].ytAddress == this.dataList[i].ytAddress){
    ...
    // 转发获取数据列表
    zfGetDataList () {
      console.log(this.zfDataForm.channelSelected)
      this.zfDataListLoading = true
      this.$http({
        url: this.$http.adornUrl('/base/tzfytrule/list'),
        method: 'post',
        data: this.$http.adornParams({
    ...
    zfDeleteHandle (val) {
      ...
      this.$confirm(`确定进行[${id ? '删除' : '批量删除'}]操作?`, '提示', {
        ...
        this.$http({
          url: this.$http.adornUrl('/base/tzfytrule/delete'),
          method: 'post',
